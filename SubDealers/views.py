...
def dac_entry(request):
    ...
    if request.method == 'POST':
        ...
        if not subdealer_code or not dac_date or not amount_raw:
            messages.error(request, "Please fill in all required DAC fields.")
            return redirect('dac_entry')

        try:
            amount = Decimal(amount_raw)
        except InvalidOperation:
            messages.error(request, "Enter a valid amount for DAC entry.")
            return redirect('dac_entry')

        try:
            subdealer = Subdealer.objects.get(subdealerCode=subdealer_code)
        except Subdealer.DoesNotExist:
            messages.error(request, "Selected subdealer does not exist.")
            return redirect('dac_entry')
        if dac_date:
        ...
        messages.success(request, "DAC entry saved successfully.")
        return redirect('dac_entry')

    return render(request, "SubDealers/dac_entry.html", {"today": timezone.now(), "subdealers": subdealers})
...
def view_dac(request):
    ...
    if subdealer_filter:
        entries = entries.filter(subdealer__id=subdealer_filter)
    ...
    return render(request, 'SubDealers/view_dac.html', context)


def monthly_summary(request):
    year_filter = request.GET.get('year')
    invoice_qs = DailyInvoice.objects.all()
    dac_qs = DACEntry.objects.all()

    if year_filter:
        try:
            year_int = int(year_filter)
            invoice_qs = invoice_qs.filter(invoice_date__year=year_int)
            dac_qs = dac_qs.filter(entry_date__year=year_int)
        except ValueError:
            year_int = None

    invoice_months = (
        invoice_qs.annotate(year=ExtractYear('invoice_date'), month=ExtractMonth('invoice_date'))
        .values('year', 'month')
        .annotate(
    ...
    invoice_quantities = (
        DailyInvoiceLineItem.objects.annotate(
    ...
    quantity_by_month = {
        (item['year'], item['month']): item['total_quantity']
        for item in invoice_quantities
    }

    dac_months = (
        dac_qs.annotate(year=ExtractYear('entry_date'), month=ExtractMonth('entry_date'))
        .values('year', 'month')
        .annotate(total_dac=Sum('amount'))
        .order_by('-year', '-month')
    )

    rows = {}

    for inv in invoice_months:
        key = (inv['year'], inv['month'])
        rows[key] = {
            'year': inv['year'],
            'month': inv['month'],
            'label': datetime(inv['year'], inv['month'], 1).strftime('%b %Y'),
            'invoice_count': inv['invoice_count'] or 0,
            'invoice_quantity': quantity_by_month.get(key, 0) or 0,
            'total_subtotal': inv['total_subtotal'] or 0,
            'total_expenses': inv['total_expenses'] or 0,
            'total_grand': inv['total_grand'] or 0,
            'total_dac': 0,
        }

    for dac in dac_months:
        key = (dac['year'], dac['month'])
        if key in rows:
            rows[key]['total_dac'] = dac['total_dac'] or 0
        else:
            rows[key] = {
                'year': dac['year'],
                'month': dac['month'],
                'label': datetime(dac['year'], dac['month'], 1).strftime('%b %Y'),
                'invoice_count': 0,
                'invoice_quantity': 0,
                'total_subtotal': 0,
                'total_expenses': 0,
                'total_grand': 0,
                'total_dac': dac['total_dac'] or 0,
            }

    rows_list = sorted(rows.values(), key=lambda row: (-row['year'], -row['month']))

    year_choices = sorted({row['year'] for row in rows_list}, reverse=True)

    return render(request, 'SubDealers/monthly_summary.html', {
        'rows': rows_list,
...
def dac_edit(request, entry_id):
    entry = get_object_or_404(DACEntry, pk=entry_id)
    subdealers = Subdealer.objects.all()
    ...
    if request.method == 'POST':
        ...
        if not subdealer_code or not dac_date or not amount_raw:
            messages.error(request, 'Please fill in all required DAC fields.')
            return redirect('dac_edit', entry_id=entry.id)
        ...
        entry.entry_date = datetime.strptime(dac_date, '%Y-%m-%d').date()
        entry.amount = amount
    ...
    return render(request, 'SubDealers/dac_edit.html', {'entry': entry, 'subdealers': subdealers})
...
def view_invoices(request):
    ...
    return render(request, 'billing/view_daily_sell_invoices.html', {
        'invoices': invoices,
...
def payment_verification(request):
    ...
    pending_payments = DailyInvoiceLineItem.objects.filter(
        payment_status='PENDING',
        created_at__date=today,
    ).select_related('invoice', 'subdealer', 'product').order_by('created_at', 'subdealer__name')

    return render(request, 'billing/payment_verification.html', {
...
def verify_payment(request, item_id):
    ...
    line_item = get_object_or_404(DailyInvoiceLineItem, pk=item_id, payment_status='PENDING')
    line_item.payment_status = 'PAID'
    line_item.save()
    messages.success(request, f'Payment for invoice {line_item.invoice.invoice_number} verified successfully.')
    return redirect('payment_verification')


def due_payments(request):
    threshold = timezone.now() - timedelta(days=1)
    due_items = DailyInvoiceLineItem.objects.filter(
        payment_status='PENDING',
        created_at__lt=threshold
    ).select_related('invoice', 'subdealer', 'product').order_by('created_at')

    return render(request, 'billing/payment_due_list.html', {
        'due_items': due_items,
...
def download_invoice_pdf(request, invoice_id):
    ...
    pdf.drawString(x + 300, y, f'Total Quantity: {invoice.line_items.aggregate(total_qty=Sum("quantity"))["total_qty"] or 0}')
...
def print_invoice(request, invoice_id):
    invoice = get_object_or_404(DailyInvoice, pk=invoice_id)
    total_qty = invoice.line_items.aggregate(total_qty=Sum('quantity'))['total_qty'] or 0
    ...
    return render(request, 'billing/print_daily_sell_invoice.html', {
        'invoice': invoice,
        'total_qty': total_qty,
...
def mismatch_records(request):
    mismatch_items = []
    for item in DailyInvoiceLineItem.objects.select_related('invoice', 'subdealer', 'product').filter(
...
def print_mismatch_record(request, invoice_id):
    ...
    return render(request, 'billing/print_mismatch_record.html', {
        'invoice': invoice,
        'mismatch_items': mismatch_items,
